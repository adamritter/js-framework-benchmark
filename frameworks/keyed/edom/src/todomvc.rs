...
fn footer(container, todolist :&mut Vec<TodoItem>, num_active: usize) {
    container.footer(|footer| {
        footer.span(|span| {
            span.class("todo-count");
            span.strong().text(num_active)
            span.text(if num_active==1 {"item left"} : {"items left"});
        })
        footer.ul(|ul| {
            ul.li(|li| {
                li.a("#/").class(if currentFilter==ShowState::ALL {"selected"} : {""})
            }
            ul.li(|li| {
                li.a("#/active").class(if currentFilter==ShowState::ACTIVE {"selected"} : {""})
            }
            ul.li(|li| {
                li.a("#/completed").class(if currentFilter==ShowState::COMPLETED {"selected"} : {""})
            }
        });
